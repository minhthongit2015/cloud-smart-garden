...
module.exports = class extends CRUDService {
  ...
  static async create({ state, station, doc }) {
    const record = doc || { state, station: this.getId(station._id) };
    return super.create.call(this, { doc: record });
  }
  ...
  static async generateRecordByDate(date, stationId, next = this.nextTime) {
    ...
    return Promise.all(
      records.map(record => this.create({ doc: record }))
    );
  }
